...
public interface ChapterRepository extends JpaRepository<Chapter, Long> {
  ...
  @Query(
      "SELECT DISTINCT chptr from Chapter chptr left join fetch chptr.course left join fetch chptr.lectures lctrs left join fetch lctrs.lectureResource left join fetch lctrs.tags where chptr.course.id=?1")
